  ...
  return <article className="product-card">
    <div className="product-main">
      ...
      <div className="product-details">
        <div className="product-heading"><h3>{item.name}</h3><span className="product-price">{formatMoney(price)}</span></div>
        <p className="product-description">{item.description}</p>
        <div className="size-row"><span>Size</span><div className="size-options">{sizeOptions.map((option) => <button type="button" className={size === option ? 'size-option is-selected' : 'size-option'} onClick={() => { setSize(option); setQuantity(1) }} key={option}>{option}</button>)}</div></div>
      </div>
    </div>
